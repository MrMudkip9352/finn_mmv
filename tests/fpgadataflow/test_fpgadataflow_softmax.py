...
def make_single_hwsoftmax_modelwrapper(
    impl_style="hls", simd=1, idt=DataType["UINT8"], ifm_dim=(128, 128)
):
    """
    ...
    graph = helper.make_graph([new_node], "softmax_graph", inputs=[inp], outputs=[outp])
...
def test_fpga_dataflow_hwsoftmax(impl_style, simd, idt, exec_mode, ifm_dim):
    ...
    io_shape = ifm_dim
    tollerance = 1e-5
    model = make_single_hwsoftmax_modelwrapper(
        impl_style=impl_style, simd=simd, idt=idt, ifm_dim=ifm_dim
    )
    ...
    for i in range(len(y_hw_flat)):
        if np.allclose(y_hw_flat[i], y_ref_flat[i], atol=tollerance) is False:
